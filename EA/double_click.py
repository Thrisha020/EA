    ...
    callback = CustomJS(args=dict(source=bokeh_graph.node_renderer.data_source), code="""
        ...
        if (indices.length > 0) {
            ...
            if (typeof window.clickTimeout !== 'undefined') {
                ...
                delete window.clickTimeout;

                // Double-click detected
                console.log(`Double-clicked on node: ${selected_node}`);
                // Trigger logic for double-click (e.g., fetch dependencies)
                fetch(`/process_double_click?node=${selected_node}`)
                ...
                window.clickTimeout = setTimeout(() => {
                    console.log(`Single-clicked on node: ${selected_node}`);
                    // Trigger logic for single-click (e.g., open file in VSCode)
                    fetch(`/process_single_click?node=${selected_node}`)
                        .then(response => response.json())
                        .then(data => console.log(data))
                        .catch(error => console.error('Error:', error));
